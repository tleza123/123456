/* Portable reference calculation engine for the Firebase plan; money is integer satang. */
...
function validateEmployeeDates_(employee) {
  ...
  if (employee.endDate) requireValue_(dateKey_(employee.endDate) >= employee.startDate, 'INVALID_EMPLOYMENT_DATES');
}

function isWorkday_(date, weekdays, overrides) {
  dateKey_(date);
  requireValue_(Array.isArray(weekdays) && weekdays.every(function (d) { return Number.isInteger(d) && d >= 0 && d <= 6; }) && new Set(weekdays).size === weekdays.length, 'INVALID_CALENDAR');
...
function rateOn_(sortedRates, date) {
  var selected = null;
  sortedRates.forEach(function (r) { if (r.effectiveFrom <= date) selected = r; });
...
/**
 * Pure engine: caller loads records and calendar, authenticates, selects snapshots
 * for closed months, and checks calendar history before calling this function.
 * input: month, today, employee, rates[], attendance[], extras[], weekdays[],
 * calendar{}, systemStartDate. Date fields are ISO date-only strings.
 */
function calculateEmployeeMonth_(input) {
  ...
  input.attendance.filter(function (r) { return r.employeeId === employee.employeeId && r.dateKey.slice(0, 7) === input.month; }).forEach(function (r) {
    dateKey_(r.dateKey);
  ...
  var result = { employeeId: employee.employeeId, month: input.month, full: 0, half: 0, absent: 0, pending: 0,
    workedDays: 0, paidDayUnits: 0, baseSatang: 0, extraSatang: 0, totalSatang: 0, days: [], extras: [] };
  ...
  input.extras.filter(function (x) { return x.employeeId === employee.employeeId && x.monthKey === input.month; }).forEach(function (x) {
...
module.exports = { dateKey_, monthKey_, moneySatang_, monthDates_, dailyPay_, calculateEmployeeMonth_ };
